...
<div class="container-fluid">
	...
	<!-- <p>Highest Bid: <b id="hbid"><?php  number_format($start_bid,2) ?></b></p> -->
	<p>Description:</p>
    ...
    $('#manage-bid').submit(function(e){
    	e.preventDefault()
            ...
            if(parseFloat(latest)  < $('[name="bid_amount"]').val()){
            	alert_toast("Bid amount must be Lower than the current Highest Bid.",'danger')
            	end_load()
            	return false;
            }
            $.ajax({
                ...
                success:function(resp){
                    if(resp==1){
                    	...
                    	alert_toast("You have Submitted",'danger')
            			end_load()
                    }
                }
            })
